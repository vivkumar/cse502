...
// get the id of the current hclib worker
#define MY_INDEX (hclib::current_worker())
...
int main(int argc, char **argv) {
  hclib::launch([&]() {
    ...
    total_asyncs = new uint64_t[hclib::num_workers()];
    // initalize the array
    ...
    uint64_t total=0;
    for(int i=0; i<hclib::num_workers(); i++) total += total_asyncs[i]; 
